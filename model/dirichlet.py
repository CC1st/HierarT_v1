...
class MLE_Dirchlet(object):
    ...
    def get_entity_occ_times(self, trainQuads): # 统计每个实体点在每个时间戳上出现的次数
    ...
    def get_relations_observed_data(self, trainQuads):  # 获得关系在各个时段所连接的所有尾实体的总数
        relations_observed_data = {}  # key: relation, value: list of observed data
        for quad in trainQuads:
            if quad[1] not in relations_observed_data.keys():   # 全时间段上关系和tail enetiy组合出现的次数
            ...
            for time in occ_times.keys():
                if time >= quad[3]:
                    continue
                observed[(quad[3] - time) // self.timespan] = occ_times[time]   # 将与relation相关的tail entity在历史time时刻出现的次数写入observed
            relations_observed_data[quad[1]].append(observed)

            reversed_r = quad[1] + 1 + self.num_r
            if reversed_r not in relations_observed_data.keys():    #全时间段上关系和 source enetity组合在某时刻出现的次数
