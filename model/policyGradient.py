...
class PG(object):
    ...
    def calc_cum_discounted_reward(self, rewards, gamma=None):
        if not gamma:
    ...
    def entropy_reg_loss(self, all_logits):
        ...
        for logit in all_logits:
            entropy_loss.append(torch.sum(torch.mul(torch.exp(logit), logit), dim=1))
        entropy_loss = torch.stack(entropy_loss, dim=1)
